...
$a = $_SESSION['s'];
				...
				$sql = "SELECT strHouseholdLname, concat(strBuildingNo,' ',strStreet,', Purok ',strPurok) as 'Address',strResidence , intHouseholdNo FROM `tblhousehold` where !(strStatus = 'Disabled') && strHouseholdLname = '$a' ";
				...
				<td><?php echo $row->strResidence?></td>
					...
					<div class="btn-group" role="group"> 
					<button class="btn btn-primary btn-circle view" type = submit value = <?php echo $row->intHouseholdNo?> name = 'Edit' >View</button>
					
					</div>	
					<div class="btn-group" role="group"> 
					 <button  class="btn btn-success" type = submit value = <?php echo $row->intHouseholdNo?> onClick="return confirm(
  'Do you really want to perform this action?');" name = 'Move' >Move Out</button>
			
					</div>	
					<div class="btn-group" role="group">
				
					 <button class="btn btn-success" type = submit value = <?php echo $row->intHouseholdNo?> name = 'transfer' >Transfer</button></td>
					</div>	
					</div>
